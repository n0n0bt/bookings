...
// SearchAvalabilityByDates returns true if avalability exists for roomID, and false if no avalability
func (m *postgresDBRepo)SearchAvalabilityByDates(start, end time.Time, roomID int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
